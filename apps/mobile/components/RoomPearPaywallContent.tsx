...
export default function RoomPearPaywallContent({
  onClose,
    ...
    >
      ...
      <View style={[s.ctaWrap, { paddingBottom: Math.max(insets.bottom, 16) + 10 }]}>
        <TouchableOpacity style={s.ctaBtn} onPress={confirmPlan} activeOpacity={0.88}>
          ...
          <Text style={s.ctaSub}>
            {selectedMeta.title} · {selectedMeta.price}
          </Text>
        </TouchableOpacity>
        ...
        {onTryNativePurchaseFlow && (
          <TouchableOpacity
            style={s.nativeLink}
            onPress={() => void onTryNativePurchaseFlow()}
            activeOpacity={0.7}
          >
...
const s = StyleSheet.create({
  ...
  benefitsCard: {
    ...
    marginBottom: 20,
    borderWidth: 1,
    borderColor: 'rgba(180,130,60,0.15)',
    shadowColor: '#C84200',
    shadowOpacity: 0.1,
